...
if (isset($_POST['login'])) {
    ...
    if ($LogDetls == "Volunteer") {
        // $username = $_POST['uid'];
        $q1 = "SELECT * FROM `family_registration` WHERE `Mobile_No`=$username";
        $e1 = mysqli_query($conn, $q1);
        
        if (mysqli_num_rows($e1) > 0) {
            // $_SESSION['usr'] = $username;
            $fetch = mysqli_fetch_row($e1);
            header("location:family_card.php?x=$fetch[2]");
        } else {
            echo "invalid login details";
        }
    } else if ($LogDetls == "Doctor") {
        // $username = $_POST['uid'];
        $q1 = "SELECT * FROM `family_registration` WHERE `Family_Registration_No`='$username'";
        $e1 = mysqli_query($conn, $q1);
